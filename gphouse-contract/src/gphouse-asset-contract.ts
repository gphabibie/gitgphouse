...
export class GphouseAssetContract extends Contract {
    ...
    @Transaction()
    public async createGphouseAsset(ctx: Context, gphouseAssetId: string, location: string, owner: string, nocerti: string): Promise<void> {
        ...
        if (transientMap.get('price')) {
            await ctx.stub.putPrivateData('productionPrice', gphouseAssetId, transientMap.get('price'));
        }
    }
    ...
    public async readGphouseAsset(ctx: Context, gphouseAssetId: string): Promise<gphousesAsset> {
        ...
        try {
            const privBuffer = await ctx.stub.getPrivateData('productionPrice', gphouseAssetId);
            gphouseAsset.price = privBuffer.toString();
            return gphouseAsset;
        } catch (error) {
            return gphouseAsset;
        }
    ...
    @Transaction()
    public async updateGphouseAsset(ctx: Context, gphouseAssetId: string, location: string, owner: string, nocerti: string): Promise<void> {
        const hasAccess = await this.hasRole(ctx, ['Goverment']);
        if (!hasAccess) {
            throw new Error(`Only goverment can update house asset`);
        }
        const exists = await this.gphouseAssetExists(ctx, gphouseAssetId);
        if (!exists) {
            throw new Error(`The gphouse asset ${gphouseAssetId} does not exist`);
